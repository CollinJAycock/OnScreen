...
func TestBuildLadder_AV1ScalesBelowHEVC(t *testing.T) {
	...
	// AV1 < HEVC < H.264 at the same rung.
	if !(av1[0].BitrateKbps < hevc[0].BitrateKbps && hevc[0].BitrateKbps < h264[0].BitrateKbps) {
		t.Errorf("expected AV1(%d) < HEVC(%d) < H.264(%d)", av1[0].BitrateKbps, hevc[0].BitrateKbps, h264[0].BitrateKbps)
	}
}
...
func TestBuildMasterPlaylist_AV1Codecs(t *testing.T) {
	...
	if n := strings.Count(master, "CODECS=\""+codecs+"\""); n != len(rends) {
		t.Errorf("got %d CODECS attrs, want %d:\n%s", n, len(rends), master)
	}
	if !strings.HasPrefix(codecs, "av01.0.") || !strings.Contains(codecs, "mp4a.40.2") {
		t.Errorf("AV1 codecs string looks wrong: %q", codecs)
	}
	// 4K ladder → seq_level_idx 13 (5.1).
	if !strings.Contains(codecs, "13M") {
		t.Errorf("4K AV1 codecs should advertise level 13M, got %q", codecs)
	}
}
