...
// where each spine entry is one "page" in the server-side count.
//
...
// series-and-issue hierarchy in v2.1; that's a v2.x story matching
// the audiobook author/series follow-up). Page count is stashed on
// media_items.duration_ms (re-purposed: "duration" for a book = total
// pages or, for EPUB, total spine sections). 0 means the file was
// unreadable or empty — surfaced in the API as duration_ms=0 and
// rendered as "empty" in the reader.
...
// countBookPages dispatches on extension to the right counter. CBZ +
// CBR walk the archive's image entries; EPUB counts spine itemrefs.
// Anything else returns 0 (caller stores 0 as "unreadable / empty").
func countBookPages(path string) int {
	switch {
	case isEPUB(path):
		return countEpubPages(path)
	case isCBR(path):
		return countCBRPages(path)
	default:
		return countCBZPages(path)
	}
}

// readFirstBookCover dispatches cover extraction by extension. EPUB
// pulls from the OPF manifest's cover-image item; CBR + CBZ take the
// first alphabetically-sorted image entry. Returns ok=false when the
// file is unreadable or has no candidate cover image.
func readFirstBookCover(path string) ([]byte, bool) {
	switch {
	case isEPUB(path):
		return readFirstEpubCover(path)
	case isCBR(path):
		return readFirstCBRPage(path)
	default:
		return readFirstCBZPage(path)
	}
}
