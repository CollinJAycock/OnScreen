...
- R8 minification and resource shrinking re-enabled — APK shrinks ~43% (8.3 MB → 4.8 MB). Previous "blank window on launch" soak failure fixed in `proguard-rules.pro`: keep `@AndroidEntryPoint` / `@HiltAndroidApp` classes verbatim (no `allowobfuscation`, which was renaming `MainActivity` and breaking Hilt's reflective lookup), and explicitly keep `androidx.datastore.**` so the synthetic flow accessors that drive `prefs.hasServer.first()` survive shrinking
- Picture-in-picture support removed (TV-IE compliance — was entering PiP from `onUserLeaveHint`, which isn't an explicit user action)
- `RECORD_AUDIO` permission dropped — was declared but unused; carrying it triggered Play Console's microphone-permission scrutiny path for zero feature benefit
- `MainActivity` initial-route race fixed: the prefs-read coroutine now guards against the activity having saved state before it commits a fragment transaction (`isStateSaved` check + `commitAllowingStateLoss` on the initial route)
- Bundle versionCode bumped 2→3, versionName 1.0.1→1.0.2

### Web

- `/privacy` and `/account-deletion` pages — both publicly accessible without auth — for the Play Console privacy-policy and data-deletion URL fields. Layout's `isAuthPage` gate extended so they render bare, like `/login` and `/setup`
...
- Live TV DVR (Phase B): user-defined recording rules (`once` / `series` / `channel_block`), a matcher worker that runs every minute against the upcoming 48h EPG window and upserts `scheduled` recordings into the `recordings` table (migration `00044_dvr.sql`), and a recording worker that tunes each channel via the existing Live-TV Service, captures MPEG-TS with ffmpeg stream-copy to MP4 (keeping CPU near zero), and on completion creates a `media_items` row so the recording surfaces in the user's DVR library and plays through the standard `/watch/{id}` path. Schedules support priority, per-recording pre/post padding (60s/180s default), series "new-only" filter (original_air_date ≥ NOW() - 7d), and per-schedule retention days. Recordings page at `/tv/recordings` with Upcoming / Recording now / Completed / Rules tabs; each guide-cell modal grows a "Record" button that creates a `once` rule. Background EPG refresh task (`epg_refresh`) respects per-source intervals; DVR matcher task (`dvr_match`) runs every minute via the scheduled-tasks subsystem
- Live TV polish: HDHomeRun auto-discovery via UDP broadcast to `255.255.255.255:65001` (Silicondust protocol) — "Find HDHomeRun devices" button in Settings → Live TV pre-fills the Add Tuner form. Manual EPG channel mapping UI for channels auto-match missed — shows unmapped channels with a dropdown of known EPG channel IDs. Channel reorder + hide-from-guide controls using `sort_order` and `enabled` columns. Guide grid and recordings page tuner list refresh as state changes
