...
The VA-API row is split because OnScreen carries two distinct Linux Intel paths: the `*_qsv` encoder family (libmfx → VA-API → kernel `i915` → `/dev/dri/renderD128`), and the `*_vaapi` encoder family (ffmpeg's native VAAPI wrappers with `scale_vaapi` filters in a separate planner branch in [`internal/transcode/ffmpeg.go`](../internal/transcode/ffmpeg.go)). Both are validated on Intel Arc; on the same hardware, the native VAAPI path is consistently faster than QSV/libmfx — Dune HDR 1080p first-segment 2.6 s vs 12.9 s, Interstellar HDR 1080p 6.2 s vs 15.5 s, GoodFellas 4K HDR 23.0 s vs 34.2 s. AMD AV1 encode requires an RDNA3 dGPU (Ryzen 9900X iGPU's RDNA2 VCN3 doesn't have an AV1 encoder block); the planner branch is wired and probe-gated, but `av1_amf` ⚠ until RDNA3 hardware is on the bench.
...
- **AV1 AMF (RDNA3) hardware validation** *(vs Plex / Emby paid tiers; Jellyfin partial)*. Planner enum + probe + filter routing all wired for `av1_amf` (commit 2026-05-21); probe correctly excludes the encoder on RDNA2 hardware (Ryzen 9900X iGPU returned `AMF error 30 — CreateComponent(AMFVideoEncoderHW_AV1) failed` on the 1 s probe and the encoder wasn't added). Hardware soak on Radeon 7000-series or Ryzen Phoenix 7040+ iGPU is the remaining step before flipping the matrix cell to ✅.
