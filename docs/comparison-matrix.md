...
- ✅ **Track D — Playwright suite expansion**: extends `auth-providers.spec.ts` (already shipped) with ten more spec files that lift end-to-end coverage from "smoke + auth" to "smoke + auth + security + tokens + hub + transcode + playback + policy (incl. auto-grant + view-as + content-rating) + gapless + admin + browser-driven SSO/forgot-password + IdP group-sync/dedup/replay (now incl. LDAP) (+ SSE placeholder)" — full suite is now 150 cases × chromium/firefox/webkit (139 passing, 11 env-gated, 0 failing in 2.9 min end-to-end against local dev). **`auth-providers.spec.ts` + `auth-providers-browser.spec.ts` activated 2026-05-03 against the live `docker-compose.idp.yml` stack** — all 33 cases (24 contract + 9 browser-driven, across 3 browsers) PASS, and the setup is fully script-driven: Keycloak admin REST creates the realm + OIDC client + SAML client + test user (with `firstName`/`lastName` filled so Keycloak's `VERIFY_PROFILE` required-action doesn't block the SSO login); lldap GraphQL + the container's `lldap_set_password` binary creates the LDAP user; OnScreen settings configured via PATCH `/api/v1/settings` (OIDC + SAML + LDAP + SMTP `from` address). Browser-driven additions cover **full OIDC** (click SSO → Keycloak login form → callback → cookie set → JIT user usable), **full SAML** (same shape via ACS POST — closes the v2.0 beta-soak SAML signing regression in a real browser, not just at the redirect-shape level), and **forgot-password via Mailpit** (throwaway user → POST forgot-password → poll Mailpit for the reset email → extract token → verify wrong-token rejection → consume real token + new-password login → assert replay rejection — locks the single-use boundary end-to-end). Spec was also updated to accept both 302 and 307 for the OIDC redirect (the OnScreen handler currently emits 307 via `http.StatusTemporaryRedirect`; both are valid per RFC 7231). Deeper IdP behaviours added 2026-05-03 in `auth-providers-group-sync.spec.ts` cover five security-boundary rows: **OIDC + SAML + LDAP admin group sync** (each IdP's group → OnScreen `is_admin=true` on the JIT user; LDAP setup creates the `onscreen-admins` group via lldap GraphQL with idempotent UNIQUE-constraint absorption on retry), **OIDC second-login dedup** (logging in twice via the SSO flow must not create a duplicate user row), and **SAML AuthnRequest single-use replay** (capture the form-POST to ACS via `page.route()` during the legitimate flow, replay the same body anonymously, assert RequestTracker rejection). The SAML group-sync test surfaced a real config wart worth flagging: without an explicit `Username` attribute mapper on the SAML client + a matching `saml.username_attribute` on OnScreen, JIT users land with username `G-<uuid>` because the SAML NameID is a UUID — added Username + Email attribute mappers as part of the test setup. Each test sets up its own IdP fixtures idempotently so reruns are clean. **`policy.spec.ts` extended same day** with three more per-user-policy boundaries: **auto-grant template** (library `auto_grant_new_users:true + is_private:true` → fresh registrant sees it without explicit grant), **view-as middleware** (admin GET ?view_as → 200; non-admin → 403; non-GET → 403; bogus uuid → 4xx), **content-rating ceiling** (PG-13 ceiling user can't find an over-PG-13 item via `/search` AND every `/api/v1/hub` section). The original "trending leaks R-rated content" finding turned out to be a test artifact, not a product bug — surfaced + fixed three real `DisallowUnknownFields` gaps instead. PATCH `/libraries/{id}` previously required a full body (500 on `{is_private: true}` alone), PATCH `/items/{id}` silently ignored a `content_rating` field it doesn't accept, and PUT `/users/{id}/content-rating` silently ignored the wrong field name (`content_rating` instead of `max_content_rating`) — clearing the ceiling without telling the caller. All three fixed: PATCH `/libraries/{id}` now does true partial updates with current-state fallback, and `DisallowUnknownFields` is added to PATCH `/libraries/{id}` + PATCH `/items/{id}` + PUT `/users/{id}/content-rating` so wrong field names return a clear 400 instead of a 200/204 that updated nothing. New + extended files: `security.spec.ts` (rate limiting opt-in via `E2E_RATE_LIMIT`, XSS reflection via `<script>`-in-search probe + addInitScript sentinel, path-traversal in transcode segment URLs, admin endpoint authorization for unauthenticated + non-admin); `security-tokens.spec.ts` (per-file stream-token scoping — file A's token rejected on file B via the `file_id` claim, with a **finding from writing the test**: Playwright's default `request` fixture persists session cookies across requests, and `/media/stream/{id}` accepts cookie auth too, which would mask any broken token check; test now uses a fresh anonymous request context for the probe so the token boundary is what's actually being measured. Also locks the `purpose="stream"` binding — stream token rejected as Bearer on `/api/v1/libraries`, `/api/v1/users/me/preferences`, `/api/v1/hub` — and the **fresh-per-call** contract: 3 successive `/items/{id}` requests return 3 distinct PASETO tokens, no server-side caching); `hub.spec.ts` (v2.1 hub section contract — `continue_watching` + `_tv` + `_movies` + `_other` split, `recently_added`, `recently_added_by_library`, `trending` all present; `cooccurrence` / `because_you_watched` confirmed absent); `transcode.spec.ts` (DASH-removal contract — no `manifest.mpd` API path AND no `manifest_url` in start response AND `playlist_url` IS present; HLS pipeline smoke from login → library → transcode → M3U8 → first segment 200; three concurrent sessions all return `session_id`; AV1 fMP4 with `video_copy:true` produces `#EXT-X-MAP` + `.m4s` segments — gated on `E2E_AV1_MOVIE_ID`); `playback.spec.ts` chromium-only (Play/Resume button click on `/watch/{id}`, `<video>` mounts, `readyState ≥ 2`, `currentTime > 0`, seek lands within tolerance, zero console errors); `policy.spec.ts` (admin marks library `is_private:true`, freshly-registered non-admin user's `/api/v1/libraries` omits it, library is restored to public + test user deleted in `finally`; home hub contains zero matches for `/because you watched/i`); `admin.spec.ts` (`GET /admin/backup` returns either a real ZIP — magic `50 4B 03 04`, non-trivial size, `application/zip` content-type — OR a structured 503 with the explicit `PG_DUMP_UNAVAILABLE` error code AND a remediation hint; catches the regression mode where the unavailable case silently returns a blank 500. `POST /admin/tasks/{id}/run` advances `last_run_at` within 45 s — picks the first known-fast task and polls the task list until the timestamp moves, which catches scheduler-handler regressions where run-now silently no-ops); plus `smoke.spec.ts` extension that fires a library scan and asserts `/api/v1/libraries` + `/api/v1/hub` both respond in <2s while it runs — closes the regression mode where the music-library scan saturated the postgres pool to the point readiness probes started timing out (the same TrueNAS QA incident that drove the scanner mtime+size fast-skip fix above); plus `gapless.spec.ts` fixes that activate it on chromium + firefox via `E2E_GAPLESS_ALBUM`. Side fixes shipped to make the suite robust: rate-limit env overrides (`OS_AUTH_RATE_LIMIT_PER_MIN`, `OS_TRANSCODE_START_RATE_LIMIT_PER_MIN`) so the cumulative auth load of a full Playwright run doesn't trip the prod-default 10/min limiter in dev (defaults still tight in prod). Two known leftovers surfaced and noted: (1) PATCH `/api/v1/libraries/{id}` is documented as partial-update (`*bool` for `is_private`) but the handler requires the full body — sending only `is_private` returns 500 because empty `name` fails validation (test works around by GET→mutate→PATCH; the handler fix is a follow-up). (2) `/api/v1/notifications/stream` doesn't flush initial bytes for non-browser clients on local dev — curl, Node `http`, and Playwright EventSource all see zero bytes for 30+ seconds, even though real browsers work (web app SSE is functional in production). The `sse.spec.ts` placeholder is in-tree but `test.skip(true, ...)`-gated until that's diagnosed (likely a Windows dev-build flush ordering issue or a client-header-specific middleware-wrapping bug).
