...
NVENC + QSV are both hardware-validated end-to-end: AV1 NVENC on RTX 5080 (2026-04-30), and the full QSV family — `h264_qsv` / `hevc_qsv` / `av1_qsv` — on an Intel Arc A770 box (2026-05-21) against the standard nine-movie coverage set (H.264 1080p, HEVC 1080p 10-bit, HEVC 4K HDR10, AV1 4K). The HDR-to-SDR tonemap chain (zscale-linear → tonemap=hable → zscale bt709) was confirmed firing on both paths with `color_transfer=bt709` in the output segments.

The VA-API row is split because OnScreen carries two distinct Linux Intel paths: the `*_qsv` encoder family (libmfx → VA-API → kernel `i915` → `/dev/dri/renderD128`) which the Arc validation exercised end-to-end, and the `*_vaapi` encoder family (ffmpeg's native VAAPI wrappers with `scale_vaapi` filters), which has its own planner branch in [`internal/transcode/ffmpeg.go`](../internal/transcode/ffmpeg.go) and was never selected during the Arc run — Arc preferentially exposes QSV, so `EncoderVAAPI` stayed cold. Hardware-validating the native `*_vaapi` branch needs a rig that doesn't expose QSV (typically an AMD or non-Intel GPU with a VAAPI driver, or an Arc with libmfx removed at build time). AMD AV1 encode requires an RDNA3 dGPU (Ryzen 9900X iGPU's RDNA2 VCN3 doesn't have an AV1 encoder block).
...
- **Native `*_vaapi` encoder branch validation** *(vs Jellyfin core)*. The VA-API system stack itself is green via the Arc QSV run (2026-05-21) — kernel i915 + iHD driver + `/dev/dri/renderD128` all exercised. What's still untested is OnScreen's separate `EncoderVAAPI` planner branch that emits `-c:v h264_vaapi` / `scale_vaapi=…`, since Arc preferentially exposes QSV and the planner never picks the native VAAPI path on Intel hardware. Closing this needs a rig that exposes `*_vaapi` without `*_qsv` — typically a non-Intel VAAPI-capable GPU.
