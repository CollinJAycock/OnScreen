...
| Roku                            | ⚠️ | ✅ | ✅ | ⚠️ | OnScreen: [`clients/roku/`](../clients/roku/) — BrightScript + SceneGraph. Setup → login → hub → detail → search → player wired; pairing-flow sign-in, search with type-filter chips, type-aware DetailScene for shows/seasons/albums/podcasts/audiobooks/movies, per-file stream token consumed. Outstanding: transcode negotiation (HLS handoff for HEVC / non-MP4 — direct play only today), photos viewer, favorites/history, markers + Up Next + cross-device sync in player. Jellyfin: third-party. |
...
- **TV / mobile coverage is uneven** — OnScreen has a Tauri 2 desktop client (Windows/macOS/Linux), a hardware-verified Android TV / Fire TV client (Leanback + Media3 ExoPlayer), a feature-complete LG webOS app (SvelteKit + ares-package), and a Roku app at parity on the major flows (BrightScript + SceneGraph, transcode negotiation pending). Samsung Tizen scaffold exists but its browse/play routes aren't wired yet. iOS, Apple TV, and Android phone apps don't exist. The web frontend works in those browsers as a fallback.
...
- ✅ **Track E — TV clients**:
  ...
  - **Roku** (BrightScript + SceneGraph): setup → login → hub → DetailScene → search → player + pairing flow + search with type-filter chips + per-file stream-token consumption. Transcode negotiation, photos, favorites/history, markers + Up Next + sync in player still pending — channel zip builds clean (`bsc check` + brs unit tests both pass).
  - **Samsung Tizen**: scaffold only.
