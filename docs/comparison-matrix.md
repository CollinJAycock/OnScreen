...
- **Multi-node transcode fleet** — a separate `worker` binary joins the primary's queue and can run on a box with **no shared storage**, pulling the source over HTTP with a per-file token. Plex / Emby / Jellyfin all transcode in-process on the single server; none distributes transcoding across nodes.
