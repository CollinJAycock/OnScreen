...
- **Adaptive bitrate HLS ladder** *(vs all three — now ⚠, mostly closed)*. Landed on `feat/v2.4` (unreleased) via the on-demand model: server-predicted multi-rendition master + media playlists with on-demand, frame-quantized per-rung transcode and bandwidth-aware client switching, soaked on `h264_nvenc`. Remaining gap to ✅: HEVC/AV1 rungs (H.264 `.ts` only today), encoder validation beyond NVENC, and a multi-instance worker-proxy for the per-rung subpaths. See section 2 for detail.
