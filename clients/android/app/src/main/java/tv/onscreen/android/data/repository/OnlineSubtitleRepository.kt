...
import tv.onscreen.android.data.model.OnlineSubtitle
import tv.onscreen.android.data.model.SubtitleDownloadRequest
...
@Singleton
class OnlineSubtitleRepository @Inject constructor(
    private val api: OnScreenApi,
) {
    suspend fun search(itemId: String, lang: String? = null, query: String? = null): List<OnlineSubtitle> =
        api.searchOnlineSubtitles(itemId, lang, query).data

    suspend fun download(itemId: String, fileId: String, candidate: OnlineSubtitle) {
