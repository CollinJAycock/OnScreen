...
<div class="page">
  ...
  {:else}
    ...
    <!-- Per-library "Recently Added to <Library>" strips. Falls back to
         the flat aggregate when running against an older server that
         doesn't emit recently_added_by_library. The first card of the
         first non-empty row gets autofocus when Continue Watching is
         empty so the user always lands on something interactable. -->
