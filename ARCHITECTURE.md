...
- **System (`system_config`)** — cluster-wide startup toggles surfaced under Settings ▸ System: server name, watch-history retention, TMDB rate limit, public asset cache, static-ABR enable, **scanner concurrency + missing-file grace period**, **LAN discovery on/off + UDP port**. The matching env vars remain as the **initial default / fallback** (a stored override wins), so env-configured installs are unaffected.
- **Transcode (`transcode_config`)** — global output ceilings (`max_bitrate_kbps`, `max_width`, `max_height`) **and the adaptive-bitrate ladder** (`abr` on/off, `abr_max_height` hard cap, `abr_auto_max_height` soft cap for AUTO playback) editable under Settings ▸ Transcode, alongside the existing NVENC/maxrate tuning. Restart-required; the env vars are the initial default.
- **TLS / HTTPS (`tls_config`, encrypted)** — admin-uploaded certificate + private key, served from an in-memory `tls.Config` so no cert file on disk is required (Settings ▸ Security ▸ HTTPS / TLS). `TLS_CERT_FILE` / `TLS_KEY_FILE` still win when set; UI uploads are blocked in that case so the env path stays authoritative.
